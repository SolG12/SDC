...
namespace Proyecto_SDC.Logica
{
    class ControlEquipo
    {
        ...
        {
            String SQL = "select no_orden, marca, modelo, funcionalidad, tipo, nombre, direccion, telefono from cliente join equipo on cliente.id_cliente = equipo.id_cliente where no_orden = '?1'";
        ...
        {
            String SQL = "select no_orden, marca, modelo, funcionalidad, tipo, nombre, direccion, telefono from cliente join equipo on cliente.id_cliente = equipo.id_cliente";
            DataTable ListaEquipos = mConexion.EjecutarConsulta(SQL);
            return ListaEquipos;
        }
